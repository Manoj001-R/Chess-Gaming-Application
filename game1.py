...
PIECE_VALUES = { EMPTY:0, PAWN:100, KNIGHT:300, BISHOP:300, 
ROOK:500, QUEEN:900, JOKER:1300, KING:42000 }
...
FILE_MASKS = [FILE_A, FILE_B, FILE_C, FILE_D, FILE_E, FILE_F, 
FILE_G, FILE_H]
RANK_MASKS = [RANK_1, RANK_2, RANK_3, RANK_4, RANK_5, 
RANK_6, RANK_7, RANK_8]
INITIAL_BOARD = [ WHITE|ROOK, WHITE|KNIGHT, WHITE|BISHOP, 
WHITE|QUEEN, WHITE|KING, WHITE|BISHOP, WHITE|KNIGHT, 
WHITE|ROOK,
...
def __init__(self, FEN='', rank_list=None):
 global rank_pieces, rank_pieces
 ...
 for rank in rank_list:
     rank_pieces: List[Union[Union[int, str], Any]] = []
 for p in rank:
     if p.isdigit():
...
def flip_board_v(board):
 flip = [56, 57, 58, 59, 60, 61, 62, 63,48, 49, 50, 51, 52, 53, 54, 55,
...
new_game.to_move|QUEEN
 ...
 new_game.castling_rights = remove_castling_rights(new_game, 
...
CASTLE_KINGSIDE_BLACK)
 ...
 new_game.castling_rights = remove_castling_rights(new_game, 
CASTLE_KINGSIDE_WHITE|CASTLE_QUEENSIDE_WHITE)
 if leaving_position == str2bb('e1'):
 if arriving_position == str2bb('g1'):
 new_game.board = move_piece(new_game.board, [str2bb('h1'), 
str2bb('f1')])
 if arriving_position == str2bb('c1'):
 new_game.board = move_piece(new_game.board, [str2bb('a1'), 
str2bb('d1')])
...
CASTLE_KINGSIDE_BLACK|CASTLE_QUEENSIDE_BLACK)
 ...
 new_game.board = move_piece(new_game.board, [str2bb('h8'), 
str2bb('f8')])
 if arriving_position == str2bb('c8'):
 new_game.board = move_piece(new_game.board, [str2bb('a8'), 
str2bb('d8')])
 
 # update positions and next to move
 new_game.board = move_piece(new_game.board, (leaving_position, 
arriving_position))
...
def pawn_moves(moving_piece, game, color):
 return pawn_pushes(moving_piece, game.board, color) | 
pawn_captures(moving_piece, game, color)
def pawn_captures(moving_piece, game, color):
 return pawn_simple_captures(moving_piece, game, color) | 
pawn_ep_captures(moving_piece, game, color)
def pawn_pushes(moving_piece, board, color):
 return pawn_simple_pushes(moving_piece, board, color) | 
pawn_double_pushes(moving_piece, board, color)
def pawn_simple_captures(attacking_piece, game, color):
 return pawn_attacks(attacking_piece, game.board, color) & 
get_colored_pieces(game.board, opposing_color(color))
...
def pawn_attacks(attacking_piece, board, color):
 return pawn_east_attacks(attacking_piece, board, color) | 
pawn_west_attacks(attacking_piece, board, color)
...
def pawn_double_pushes(moving_piece, board, color):
 if color == WHITE:
 return north_one(pawn_simple_pushes(moving_piece, board, color)) & 
(empty_squares(board) & RANK_4)
 if color == BLACK:
 return south_one(pawn_simple_pushes(moving_piece, board, color)) & 
(empty_squares(board) & RANK_5)
...
def remove_captured_ep(game):
 ...
 return new_board
# ========== KNIGHT ==========def get_knights(board, color):
...
def diagonal_attacks(single_piece, board, color):
 return NE_attacks(single_piece, board, color) | SW_attacks(single_piece, board, 
color)
def anti_diagonal_attacks(single_piece, board, color):
 return NW_attacks(single_piece, board, color) | SE_attacks(single_piece, board, 
color)
def bishop_attacks(moving_piece, board, color):
 atks = 0
 for piece in single_gen(moving_piece):
 atks |= diagonal_attacks(piece, board, color) | anti_diagonal_attacks(piece, 
board, color)
 return atks
def bishop_moves(moving_piece, board, color):
 return bishop_attacks(moving_piece, board, color) & 
nnot(get_colored_pieces(board, color))
# ========== ROOK ==========
...
def rank_attacks(single_piece, board, color):
 return east_attacks(single_piece, board, color) | west_attacks(single_piece, 
board, color)
...
def file_attacks(single_piece, board, color):
 return north_attacks(single_piece, board, color) | south_attacks(single_piece, 
board, color)
def rook_attacks(moving_piece, board, color):
 atks = 0
 for single_piece in single_gen(moving_piece):
 atks |= rank_attacks(single_piece, board, color) | file_attacks(single_piece, 
board, color)
 return atks
...
def queen_attacks(moving_piece, board, color):
 return bishop_attacks(moving_piece, board, color) | rook_attacks(moving_piece, 
board, color)
def queen_moves(moving_piece, board, color):
 return bishop_moves(moving_piece, board, color) | rook_moves(moving_piece, 
board, color)
# ========== JOKER ==========
...
knight_attacks(moving_piece)
def joker_moves(moving_piece, board, color):
 return queen_moves(moving_piece, board, color) | knight_moves(moving_piece, 
board, color)
# ===========================
...
def pseudo_legal_moves(game, color):
 ...
 yield (get_king(game.board, color), east_one(east_one(get_king(game.board, 
color))))
 if can_castle_queenside(game, color):
 yield (get_king(game.board, color), 
west_one(west_one(get_king(game.board, color))))
...
def has_insufficient_material(game): # TODO: other insufficient positions
 if material_sum(game.board, WHITE) + material_sum(game.board, BLACK) 
== 2*PIECE_VALUES[KING]:
 return True
 if material_sum(game.board, WHITE) == PIECE_VALUES[KING]:
 if material_sum(game.board, BLACK) == PIECE_VALUES[KING] + 
PIECE_VALUES[KNIGHT] and \
 (get_knights(game.board, BLACK) != 0 or get_bishops(game.board, 
BLACK) != 0):
 return True
 if material_sum(game.board, BLACK) == PIECE_VALUES[KING]:
 if material_sum(game.board, WHITE) == PIECE_VALUES[KING] +
PIECE_VALUES[KNIGHT] and \
 (get_knights(game.board, WHITE) != 0 or get_bishops(game.board, WHITE) 
!= 0):
 return True
 return False
def game_ended(game):
...
def alpha_beta(game, color, depth, alpha=-float('inf'), beta=float('inf')):
 ...
 for move in legal_moves(game, color):
 if verbose:
 print('\t'*depth + str(depth) + '. evaluating ' + 
PIECE_CODES[get_piece(game.board, move[0])] + move2str(move))
 
 new_game = make_move(game, move)
 [_, score] = alpha_beta(new_game, opposing_color(color), depth-1, alpha, 
beta)
 
 if verbose:
 print('\t'*depth + str(depth) + '. ' + str(score) + ' [{},{}]'.format(alpha, 
beta))
...
def parse_move_code(game, move_code):
 ...
 get_piece(game.board, move[0])&PIECE_MASK == piecevalid_moves.append(move) 
...
def get_AI_move(game, depth=2):
 if verbose:
 print('Searching best move for white...' if game.to_move == WHITE else 
...
# move = minimax(game, game.to_move, depth)[0]
 ...
 end_time = time()
 if verbose:
 print('Found move ' + PIECE_CODES[get_piece(game.board, move[0])] + ' 
from ' + str(bb2str(move[0])) + ' to ' + str(bb2str(move[1])) + ' in {:.3f} 
seconds'.format(end_time-start_time) + ' ({},{})'.format(evaluate_game(game), 
...
def find_in_book(game):
 ...
 if line.startswith(game.get_move_list()) and line.rstrip() > 
game.get_move_list():
